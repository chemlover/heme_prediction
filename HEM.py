...
def group_constraint_by_distance(oa, d0=0*angstrom, group1=1, group2=2, forceGroup=3, k=10*kilocalorie_per_mole):
...
def small_molecular_vdw_term(oa, k=4.184, vdw_info_csv="vdw.csv", periodic=False, forceGroup=5):
    ...
    smallMolecular_vdw.setCutoffDistance(0.30)
    #if periodic:
...
def small_molecular_exclude_volume_term(oa, k=5 * kilocalorie_per_mole, periodic=False, forceGroup=5):
    ...
    smallMolecular_excludeVolume.setCutoffDistance(r_excl)
    if periodic:
...
def small_molecular_interaction_to_proteins_term3(oa, k=0.1 * kilocalorie_per_mole,k_screening = 1.0, screening_length=1.0,forceGroup=10):
    ...
    N4r = Natomid[3] - 1
    k = k.value_in_unit(kilojoule_per_mole)
    LP_function = ''.join(["kSSresId*0.5*(1+tanh((0.9-(r%d+0.5)/2.0)*6))*exp(-0.5*(r%d-0.57)*(r%d-0.57))*((tanh(4*(sin(N1%d)-0.5))+1)/2.0+(tanh(4*(sin(N2%d)-0.5))+1)/2.0+(tanh(4*(sin(N3%d)-0.5))+1)/2.0+(tanh(4*(sin(N4%d)-0.5))+1)/2.0)/4*(tanh(16*(sin(N1%d)-0.86))+1)/2.0*(tanh(16*(sin(N2%d)-0.86))+1)/2.0*(tanh(16*(sin(N3%d)-0.86))+1)/2.0*(tanh(16*(sin(N4%d)-0.86))+1)/2.0*0.5*(1-tanh((r%d-0.7)*100)) +" \
                            % (i,i,i, i, i, i, i , i, i, i,i,i) for i in range(1)])[:-1]
    LP_function = f'-{k}*(' + LP_function + ");"
    LP_parameters = ''.join([f"r{i}=distance(p1, p2);\
                                 N1{i}=angle(p1, p2, p3);\
...
def set_up_forces(oa, computeQ=True, submode=0, contactParameterLocation=".", membrane_center=-0*angstrom):
    ...
    if submode == 0:
        additional_forces = [
            ...
            burial_term_ligand(oa,k=1.9 * kilocalorie_per_mole,forceGroup=21)
